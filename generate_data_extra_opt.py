...
def add_hdri_background(scene, selected_hdri):
    ...
    hdri_image = bpy.data.images.load(selected_hdri)

    # print HDRI resolution
    print(f"Loaded HDRI: {hdri_image.name} with resolution {hdri_image.size[0]}x{hdri_image.size[1]}")

    # Reduce HDRI resolution for faster processing
    # hdri_image.scale(1024, 512)  # Scale down from 8K
    ...
    # background.inputs['Strength'].default_value = 0.5  # Reduce strength for faster convergence
...
def capture_views(obj, camera, scene, depsgraph, selected_objects, selected_distractors, 
    ...
    # Pre-create output directories to avoid repeated filesystem calls
    if save_files:
    ...
    for i in range(num_pics):
        ...
        # Update scene only once per view
        bpy.context.view_layer.update()
        ...
        if save_files:            
            ...
            # Set render settings once
            scene.render.image_settings.file_format = 'JPEG'
            # scene.render.image_settings.quality = 85  # Reduce quality for speed
...
def render_setup(scene, render_percentage):
    ...
    # scene.cycles.adaptive_threshold = 0.1  # Stop early when noise is low
    # scene.cycles.adaptive_min_samples = 4  # Minimum samples before adaptive kicks in
    scene.cycles.use_denoising = True
    scene.cycles.use_progressive_refine = False
    # scene.cycles.use_progressive_refine = True  # Enable for faster preview
    scene.cycles.device = "GPU"

    scene.render.use_persistent_data = False
    # # Memory and performance optimizations
    scene.render.use_persistent_data = True  # Keep data in memory between renders
    # scene.cycles.use_auto_tile = True  # Let Cycles choose optimal tile size
    # scene.cycles.preview_samples = 4  # Fast preview
    
    # # Reduce subdivision levels for faster rendering
    # scene.cycles.dicing_rate = 4.0  # Higher = less subdivision
